...
class CaesarCipherApp:
    ...
    def create_encrypt_tab(self):
        title_label = ttk.Label(self.encrypt_frame, text="Шифр Цезаря", font=('Arial', 16, 'bold'))
        title_label.pack(pady=10)
        
        input_frame = ttk.LabelFrame(self.encrypt_frame, text="Входной текст", padding=10)
        input_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        self.input_text = scrolledtext.ScrolledText(input_frame, height=6, font=('Arial', 11), wrap=tk.WORD)
        ...
        ttk.Label(settings_frame, text="Ключ сдвига:").grid(row=0, column=0, padx=5, pady=5, sticky='e')
        ...
        ttk.Label(settings_frame, text="Режим:").grid(row=0, column=2, padx=10, pady=5, sticky='e')
        self.mode_var = tk.StringVar(value="encrypt")
        ttk.Radiobutton(settings_frame, text="Зашифровать", variable=self.mode_var, value="encrypt").grid(row=0, column=3, padx=5, pady=5)
        ttk.Radiobutton(settings_frame, text="Расшифровать", variable=self.mode_var, value="decrypt").grid(row=0, column=4, padx=5, pady=5)
        
        btn_frame = ttk.Frame(self.encrypt_frame)
        btn_frame.pack(pady=10)
        
        ttk.Button(btn_frame, text="Выполнить", command=self.process_text, width=20).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Очистить", command=self.clear_encrypt_tab, width=20).pack(side=tk.LEFT, padx=5)
        ...
        self.result_text = scrolledtext.ScrolledText(result_text_frame, height=6, font=('Arial', 11), wrap=tk.WORD)
        ...
        ttk.Button(copy_btn_frame, text="Копировать\nрезультат", command=self.copy_result, width=15).pack(pady=5)
        
        self.key_info_label = ttk.Label(self.encrypt_frame, text="", font=('Arial', 10, 'italic'))
        self.key_info_label.pack(pady=5)
    
    def create_crack_tab(self):
        title_label = ttk.Label(self.crack_frame, text="Взлом шифра Цезаря\n(метод наименьших квадратов)", font=('Arial', 16, 'bold'))
        title_label.pack(pady=10)
        
        desc_label = ttk.Label(self.crack_frame, text="Введите зашифрованный русский текст для автоматического взлома", font=('Arial', 10))
        desc_label.pack(pady=5)
        
        input_frame = ttk.LabelFrame(self.crack_frame, text="Зашифрованный текст", padding=10)
        input_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        self.crack_input = scrolledtext.ScrolledText(input_frame, height=4, font=('Arial', 11), wrap=tk.WORD)
        self.crack_input.pack(fill='both', expand=True)
        self.create_context_menu(self.crack_input)
        
        ttk.Button(self.crack_frame, text="Взломать шифр", command=self.crack_cipher, width=30).pack(pady=10)
        ...
        self.crack_result = scrolledtext.ScrolledText(result_text_frame, height=10, font=('Arial', 11), wrap=tk.WORD)
        ...
        ttk.Button(copy_crack_btn_frame, text="Копировать\nрезультат", command=self.copy_crack_result, width=15).pack(pady=5)
    ...
    def copy_crack_result(self):
        ...
        if result:
            self.root.clipboard_clear()
            self.root.clipboard_append(result)
            messagebox.showinfo("Копирование", "Результат взлома скопирован!")
